...
public class CameraBehaviour : MonoBehaviour {
	...
	void LateUpdate () {
		...
		if(player.transform.position.x > maxX && player.transform.position.y > maxY){//On check X et Y
			...
			transform.position = new Vector3 (-maxX, -maxY, offset.z);
		}else if (player.transform.position.x > maxX || player.transform.position.x < -maxX) { //On check les X
			lockX = new Vector3 (transform.position.x, player.transform.position.y, 0);
			transform.position = lockX + offset;
		}else if(player.transform.position.y > maxY || player.transform.position.y < -maxY){//On check les Y
